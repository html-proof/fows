...
export function parseYouTubePage(html) {
    ...
    if (ytInitialData) {
        try {
            dataCandidates.push(JSON.parse(ytInitialData[1]));
        } catch (_e) { /* JSON parse failed */ }
    }

    const scriptMatches = html.match(/<script[^>]*>([\s\S]*?)<\/script>/gi) ?? [];
    for (const scriptTag of scriptMatches) {
        const innerContent = scriptTag.replace(/^<script[^>]*>/i, '').replace(/<\/script>$/i, '').trim();
        if (!innerContent || !/^[\[{]/.test(innerContent)) continue;

        try {
            dataCandidates.push(JSON.parse(innerContent));
        } catch (_e) { /* JSON parse failed */ }
    }
...
export function extractYouTubePlaylistTracks(data) {
    const items = [];
    ...
    function extractText(node) {
        ...
        const candidates = [node.title, node.subtitle, node.secondaryText, node.shortBylineText, node.longBylineText, node.content];
    ...
    function walk(obj) {
        ...
        const renderer = getRenderer(obj);
        if (renderer) {
            let title = extractText(renderer.title);
            let artist = extractText(renderer.shortBylineText)
                || extractText(renderer.subtitle)
                || extractText(renderer.longBylineText)
                || '';

            const flexColumns = renderer?.flexColumns ?? [];
            const flexColumnTexts = flexColumns
                .map(column => extractText(column?.text))
                .filter(Boolean);

            if (!title && flexColumnTexts[0]) {
                title = flexColumnTexts[0];
            }

            if (!artist) {
                const nonTitleColumn = flexColumnTexts.find(columnText => columnText && columnText !== title);
                if (nonTitleColumn) {
                    artist = nonTitleColumn;
                }
            }

            if (!artist) {
                const nestedContent = extractText(renderer?.content);
                if (nestedContent) {
                    artist = nestedContent;
                }
            }

            if (!artist) {
                const secondaryText = extractText(renderer?.secondaryText);
                if (secondaryText) {
